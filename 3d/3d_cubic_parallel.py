...
t=Parallel(n_jobs=num_cores-2)\
(delayed(transmission)(a=i,cishu=j)\
 for j in range(num_sample) for i in size)

tt=np.reshape(t,(num_sample,len(size))).T  #y is size, x is different sample
#    for cishu in range(50):
#        sys = make_cuboid(a)
#    
#        gf_mode = kwant.smatrix(sys, en)
